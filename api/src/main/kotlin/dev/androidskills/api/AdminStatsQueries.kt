...
/**
 * Admin dashboard stats (Phase 4). Counts are aggregated; list endpoints are paginated and do not
 * return totals.
 */
object AdminStatsQueries {
  ...
  fun get(): AdminStats = transaction {
    ...
    val newUsers30d =
      Users.selectAll().where { Users.createdAt greaterEq cutoff30d }.count().toInt()
    val suspended =
      Users.selectAll().where { Users.status eq UserStatus.suspended.name }.count().toInt()
    val skills = Skills.selectAll().count().toInt()
    val queue =
      Submissions.selectAll()
        .where { Submissions.state inList listOf("in_review", "changes_requested") }
        .count()
        .toInt()
