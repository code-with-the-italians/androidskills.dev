...
- Astro proxy (`web/src/lib/proxy.ts`) now correctly forwards POST/PATCH/PUT bodies with `duplex: 'half'`, preserves `X-Forwarded-For`, and forwards each `Set-Cookie` individually via `getSetCookie()`.
- `TRUSTED_PROXY_COUNT` is set per Kamal role: web=2, api=1.
