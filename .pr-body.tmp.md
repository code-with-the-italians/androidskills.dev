...
- Astro proxy (`web/src/lib/proxy.ts`) now correctly forwards POST/PATCH/PUT bodies with `duplex: 'half'`, preserves `X-Forwarded-For`, forwards each `Set-Cookie` individually via `getSetCookie()`, and wraps upstream failures in a 502.
...
- Kamal is not installed locally; `kamal.yml` and `kamal.path-prefix.yml` syntax is untested and includes `REVIEW` comments for the exact Kamal-version options (no-overlap deploy, stop-grace, volume syntax, per-role env support).
- The GitHub Actions workflows are scaffolds; they will need the full secret set before the first run.
- End-to-end proxy smoke test (client → proxy → API with body, redirect, and Set-Cookie) has not been run in a real container or deploy.
